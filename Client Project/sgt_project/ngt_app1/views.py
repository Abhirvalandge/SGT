...
def view_entry(request):
    entry = NGTEntries.objects.order_by('-lr_no')
    paginator = Paginator(entry, 10)
    page = request.GET.get('page')
    sum = NGTEntries.objects.aggregate(Sum('total_balance'))
    try:
...
def add_entry(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            lr_no_exists = NGTEntries.objects.filter(lr_no__icontains=lr_no).exists()
            NGTEntries(date=date, firm_name=firm_name, lr_no=lr_no, vehicle_no=vehicle_no, location=location, amount=amount, cash=cash,
...
def entry_detail(request, lr_no):
    entry = get_object_or_404(NGTEntries, lr_no=lr_no)
    return render(request, 'detail.html', {'entry': entry})


def update_entry(request):
    x = request.POST.get('u_id')
    update = NGTEntries.objects.get(lr_no=x)
    form = forms.AddEntryForm(request.POST)
...
def delete_entry(request):
    ...
    NGTEntries.objects.filter(lr_no=x).delete()
...
def showresult(request):
    if request.method == 'POST':
        ...
        searchresult = NGTEntries.objects.raw(
...
def search(request):
    ...
    if search != '':
        search_by_vehicle_no = NGTEntries.objects.filter(vehicle_no__icontains=search)
        search_by_status = NGTEntries.objects.filter(status__icontains=search)
        search_by_lr_no = NGTEntries.objects.filter(lr_no__icontains=search)
...
def export_excel_without_search(request):
    ...
    rows = NGTEntries.objects.order_by('-lr_no').values_list('date', 'firm_name', 'lr_no', 'vehicle_no', 'location',
                                                          'amount', 'cash', 'diesel', 'rtgs', 'commission','total_balance', 'status')
    for row in rows:
        row_num += 1
        for col_num in range(len(row)):
...
def pdf_report_create(request, pk):
    entry = get_object_or_404(NGTEntries, lr_no=pk)
    template_path = 'pdf_detail.html'
    context = {'entry': entry}
    # Create a Django response object, and specify content_type as pdf
    response = HttpResponse(content_type='application/pdf')
...
def from_to_date_pdf(request):
    ...
    search_by_from_to_date = NGTEntries.objects.raw(
        'select date,firm_name,lr_no,vehicle_no,location,amount,cash,diesel,rtgs,commission,status from entries where date between "' + fromdate + '" and "' + todate + '"')
